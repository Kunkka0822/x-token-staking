...
pub struct Claim<'info> {
    ...
    #[account(
        mut,
        seeds = [VAULT_REWARD_SEED.as_bytes(), vault.to_account_info().key.as_ref()],
        bump
    )]
    /// CHECK:
        ...
        if reward_amount > 0 {
            ...
            let cpi_ctx = CpiContext::new(
                ...
                anchor_spl::token::Transfer {
                    ...
                    to: ctx.accounts.reward_mint.to_account_info().clone(),
                    authority: ctx.accounts.reward.to_account_info().clone(),
                },
            );
            anchor_spl::token::transfer(cpi_ctx.with_signer(&[&seeds[..]]), reward_amount)?;

            user.reward_earned_pending = user.reward_earned_pending
                .checked_sub(reward_amount).unwrap();
            user.reward_earned_claimed = user.reward_earned_claimed
                .checked_add(reward_amount).unwrap();
        }
    }
    Ok(())
}